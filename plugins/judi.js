let { MessageType } = require('@adiwajshing/baileys')
 ...
 async function handler(m, { conn, args, isROwner }) {
     ...
     if (m.sender in confirm) throw 'You're still gambling, wait until it's finished!!'
     try {
         let user = global.DATABASE._data.users[m.sender]
         let count = (args[0] && number(parseInt(args[0])) ? Math.max(parseInt(args[0]), 1) : /all/i.test(args[0]) ? Math.  floor(parseInt(user.money)) : 1) * 1
         if ((user.money * 1) < count) return conn.sendMessage(m.chat, button('💵You don't have enough money!!', user), MessageType.buttonsMessage, { quoted: m })
 ...
 handler.before = async m => {
     ...
     try {
         if (/^y(es|a)?$/i.test(txt)) {
             ...
             let You = (Math.floor(Math.random() * 86)) * 1
             let status = 'Lost'
             if (Bot < You) {
                 user.money += count * 1
                 status = 'Win'
 ...
 You *${status}*, you ${status == 'Win' ?  `Getting *+${count * 2}*` : status == 'Lost' ?  `Loss *-${count * 1}*` : `Gain *+${Math.floor(count / 1.5)}*`} Money
     `.trim())
             clearTimeout(timeout)
             delete confirm[m.sender]
             returns !0
         ...
         clearTimeout(timeout)
         delete confirm[m.sender]
         if (moneyFirst > (user.money * 1)) user.money = moneyFirst * 1
         m.reply('Error while gambling (Rejected)')
         returns !0
     } finally {
         clearTimeout(timeout)
         delete confirm[m.sender]
 ...
 function button(text, user) {
     ...
     let adventure = new Date - user.lastventure > 300000
