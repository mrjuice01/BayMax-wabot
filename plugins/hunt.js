let handler = async(m, { conn, text }) => {
 ...
 player.healt -= dmg
 player.lasthunt = new Date * 1 // hunt time 2min

 if (player.health < 0) {
 let msg = `*${pname}* You Die Killed By ${monsterName}`
 if (player.level > 0) {
 player.level -= 1
 msg += `\nYour Level Dropped by 1 For Dying While Hunting!`
 }
 player.health = 100
 m.reply(msg)
 return
 ...
 function MeNit(ms) {
 let m = isNaN(ms) ?  '02' : Math.floor(ms / 600000) % 60
